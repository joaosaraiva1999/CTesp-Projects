...
$(document).ready(function () {
    let $stickyButton = $('.sticky-button')
    let $footer = $('footer')

    function parar() {
        let parar_butao = $(window).scrollTop() + $(window).height() - $($footer).offset().top
        $stickyButton.css('bottom', parar_butao > 0 ? parar_butao + 20 : 20)
    }

    $(window).on('scroll resize', parar)
    parar();
})